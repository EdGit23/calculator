...
class App(tk.Tk):
	def __init__(self):
		...
		self.empty_string = 'SYNTAX ERROR'
		...
		#frame3
		self.button1 = ttk.Button(self.frame3,text='x^2',width=9,padding=1, command=lambda:self.calculate_method('x2')).grid(row=0, column=0)
		self.button2 = ttk.Button(self.frame3,text='x^3',width=9,padding=1).grid(row=0, column=1)
		self.button3 = ttk.Button(self.frame3,text='shift',width=9,padding=1).grid(row=0, column=2)
		self.button4 = ttk.Button(self.frame3,text='log',width=9,padding=1).grid(row=0, column=3)

		#frame4
		self.button1 = ttk.Button(self.frame4,text='AC',width=9,padding=1, command=self.clear_screen).grid(row=0, column=0)
		...
		self.button3 = ttk.Button(self.frame4,text='DEL',width=9,padding=1, command=self.delete_characters).grid(row=0, column=2)
	...
	def calculate_method(self,char_x):
		...
		if self.char_x == 'x2':
			...
			self.display_window.insert('1.0', eval(self.num1) ** 2)
	...
	def equals(self):
		...
		#Check to see if only one number has been passed and just print it else return error message
		#Mostly a newline character check should do the trick

		if self.display_window.get('1.0', tk.END) == '\n':
			self.display_window.insert('1.0', self.empty_string)
